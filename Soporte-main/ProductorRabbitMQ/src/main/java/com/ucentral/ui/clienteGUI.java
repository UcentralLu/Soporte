...
public class clienteGUI extends JFrame {
    ...
    private void initUI() {
        ...
        gbc.gridwidth = 2;
        gbc.anchor = GridBagConstraints.WEST;
        JCheckBox globalCheck = new JCheckBox("Mensaje Global");
        panel.add(globalCheck, gbc);
        ...
        sendBtn.addActionListener(e -> sendTicket(globalCheck.isSelected()));

        add(panel);
    }

    private void sendTicket(boolean esGlobal) {
        String userId = userIdField.getText().trim();
        String description = descriptionArea.getText().trim();
        String severity = (String) severityBox.getSelectedItem();
        String area = (String) areaBox.getSelectedItem();

        if (userId.isEmpty() || !userId.matches("^[a-zA-Z0-9]+$") || description.length() < 10) {
            JOptionPane.showMessageDialog(this,
                    "El ID de usuario debe ser alfanumérico y la descripción debe tener al menos 10 caracteres.",
        ...
        try {
            ...
            if (esGlobal) {
                producer.sendGlobalTicket(ticketJson);
            } else {
